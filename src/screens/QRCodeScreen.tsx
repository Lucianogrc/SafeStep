...
export default function QRCodeScreen() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (!snap.exists()) {
          // Si por alguna razón no existe el documento, salimos
          console.warn("Documento de usuario no existe en 'users'");
          return;
        }
        ...
        // ID visible (puedes cambiar el formato si quieres)
        const visibleId =
          data.userId || `HST-${currentUser.uid.substring(0, 6).toUpperCase()}`;
        setUserId(visibleId);

        // 🔑 Asegurar que haya un qrValue en Firestore
        let value = data.qrValue as string | undefined;

        if (!value) {
          // Generamos uno nuevo si no existe
  ...
  // 🔄 Rotación animada + regenerar y actualizar en Firestore
